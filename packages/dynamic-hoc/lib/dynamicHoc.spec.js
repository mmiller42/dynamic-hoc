...
test('names the wrapping component using the HOC factory name and component name', t => {
  ...
  const testInstance = render(t, Wrapper, {})
...
test('names the wrapping component using the given HOC factory name and component name', t => {
  ...
  const testInstance = render(t, Wrapper, { x: 'y' })
...
test('names the wrapping component using the HOC factory name and component displayName', t => {
  const Wrapper = dynamicHocWithArgNames(hocFactory)('foo', 'bar')(
...
test('names the wrapping component using an empty factory name and component name', t => {
  const Wrapper = dynamicHocWithArgNames(anonymousHocFactory)('foo', 'bar')(
    Component,
  )
  const testInstance = render(t, Wrapper, {})
...
test('executes the HOC with original args and renders', t => {
  ...
  t.deepEqual(getWrappedElement(testInstance).props, {
    x: 'y',
    arg1: 'foo',
    arg2: 'bar',
  })
...
test('does not affect more than one wrapped component', t => {
  ...
  const testInstance1 = render(t, Wrapper1, { x: 'y' })
  const testInstance2 = render(t, Wrapper2, { x: 'z' })

  act(() => {
    Wrapper1.args.arg1 = 'baz'
  })

  t.deepEqual(getWrappedElement(testInstance1).props, {
    x: 'y',
    arg1: 'baz',
    arg2: 'bar',
  })
  t.deepEqual(getWrappedElement(testInstance2).props, {
    x: 'z',
    arg1: 'foo',
    arg2: 'bar',
  })
